...
describe "ActiveRecord American Gladiator" do
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      # Changeable Start
      orders = Order.joins(:items).select("orders.*").where("items.name = ? OR items.name = ?","Football", "Wrestling Ring")
      # chosen = ["Football","Wrestling Ring"]
      # orders = Order.joins(:items).select("orders.*").where("items.name": chosen) #which is better?
      # Changeable End
